...
import com.helper.starter.bean.ProxySimpleJob;
import com.helper.starter.strategy.LocalJobStrategy;
...
/**
 * 在spring.factories中配置的EnableAutoConfiguration实现类，意味着spring会自动识别这个类，并按条件加载
 */
@Configuration
//如果BeanFactory中，有bean持有这个注解，则符合条件。故在启动类添加该注解则启动，不添加则不启动.
@ConditionalOnBean(annotation = EnableEasyJobConfiguration.class)
//这个注解是让ConfigurationProperties的类加载加来
@EnableConfigurationProperties({EasyConfig.class,ZookeeperEasyConfig.class, EasyJobEventConfiguration.class})
public class EasyJobAutoConfiguration implements ApplicationContextAware, InitializingBean {
    private final Log logger = LogFactory.getLog(getClass());
    private ConfigurableApplicationContext applicationContext;

    public EasyJobAutoConfiguration() {
        System.out.println("EasyJobAutoConfiguration");
    }
